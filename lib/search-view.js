...
export default class GrepView {
    constructor() {
        ...
        this.selectListView = new SelectListView({
            items: [],
            maxResults: 10,
            emptyMessage: "Search result is empty.",
        ...
        // this.subscriptions.add(atom.commands.add(this.filterEditorView.element, 'search-everywhere:pasteEscaped', this.pasteEscaped));
    ...
    grepProject(query) {
        console.log(query);
        console.log(this.minFilterLength);
        if (this.minFilterLength && (query.length < this.minFilterLength)) {
            this.selectListView.update({
                items: [],
                loadingMessage: `Please enter more than ${this.minFilterLength} chars`}
            );
            return;
         }
        ...
        .then((items) => {
            console.log("Result:");
            console.log(items);
            this.selectListView.update({items: items, loadingMessage: "loading..."});
        }).catch((error) => {
            console.log(error);
            this.selectListView.update({items: [], loadingMessage: error});
        });
    }
...
class ListItemComponent {
  ...
  render () {
      ...
      const displayedPath = showFullPath ? this.item.filePath : path.basename(this.item.filePath);
      return (
          <li class="two-lines">
          ...
          <span>{c1}</span>
          <span class="text-info matched-search">{c2}</span>
          <span>{c3}</span>
          </div>
          </li>
      );
